# @network/tls — Implementation Plan

A TLS 1.3 (with 1.2 fallback) client over `@network/transport`. Implement in this
order; each step is independently testable. All crypto goes through
`@network/crypto` — never `node:crypto` directly.
...
1. Generate key shares via `@network/crypto` for each group in the profile.
...
1. `hkdfExpandLabel` over the @network/crypto HKDF primitives.
...
AEAD primitives from `@network/crypto`. Handle TLS 1.3 inner content-type wrapping and
...
server with minVersion TLS 1.3) over a loopback `@network/transport` and run a full
...
- [ ] Chain verification delegates signatures to @network/crypto.
