/**
 * Tests for @browsercore/compression Node backend.
 *
...
import { nodeCompression, NodeZlibCompressionProvider } from "../../../../src/platform/compression/node/compression.js";
import {
    assertNever,
    SUPPORTED_ENCODINGS,
...
describe("default singleton", () => {
    it("exports a NodeZlibCompressionProvider instance as the default", () => {
        expect(nodeCompression).toBeInstanceOf(NodeZlibCompressionProvider);
    });

    it("is a usable CompressionProvider (gzip round-trip works)", () => {
        const payload = detBuffer(256);
        const roundTrip = nodeCompression.gunzip(nodeCompression.gzip(payload));
